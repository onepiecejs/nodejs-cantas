...
(function (exports) {
  ...
  exports.addupSuccessCount = function(dataSource) {
    var count = 1 + dataSource.lists.length + dataSource.cards.length + dataSource.checklists.length;

    for(var i=0; i<dataSource.actions.length; i++) {
      if(dataSource.actions[i].type === 'commentCard') {
        count += 1;
      }
    }

    for(var i=0; i<dataSource.checklists.length; i++) {
      count += dataSource.checklists[i].checkItems.length;
    }

    for(var i=0; i<dataSource.cards.length; i++) {
          ...
          newCard.save(function(err, createdCard) {
            if(err) {
              ...
              if(dataSource.checklists.length > 0) {
                createChecklist(userId, createdCard.id, card.id, updatedBoard, dataSource, callback);
              }
              if(dataSource.actions.length > 0) {
                createComment(userId, createdCard.id, card.id, updatedBoard, dataSource, callback);
              }

              createCardLabelRelation(card.labels, createdCard.id, updatedBoard, dataSource, callback);
      ...
      async.whilst(
        ...
        function (cb) {
          ...
          newComment.save(function(err, createdComment) {
            if(err) {
              callback('Creating the comment presented by '+ comment.memberCreator.username +' failed');
            }
            else {
              callback(null);
              cb(null);
  ...
  function createCardLabelRelation (labels, cardId, updatedBoard, dataSource, callback) {
    ...
    async.whilst(
      ...
      function (cb1) {
        ...
        async.waterfall([
          function(cb2){
            Label.findOne({ 'boardId': updatedBoard.id, 'order': order }, function (err, queriedLabel) {
              if(err) {
          ...
          newChecklist.save(function(err, createdChecklist) {
            if(err) {
              ...
              if(checklist.checkItems.length >0) {
                createChecklistItem(userId, checklist.checkItems, createdChecklist.id, cardId, updatedBoard, callback);
              }
              cb(null);
            }
          });
  ...
  function createChecklistItem (userId, checklistItems, checklistId, cardId, updatedBoard, callback) {
