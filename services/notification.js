 ...
 */
module.exports.notify = function(socket, to, msg, type, options) {
  if (!socket || !to || !msg){
    console.error("Error: function 'notify' missing arguments");
    return;
  }

  var type = type || NotificationTypes.information;
  var options = options || {};
  ...
  if (!options.from) options.from = socket.handshake.user.email;
  if (!options.to) options.to = to.email;
  if (!options.subject) options.subject = "Notification";
  if (!options.body) options.body = markdown.toHTML(msg);
  if (!options.template) options.template= "notification.jade";
