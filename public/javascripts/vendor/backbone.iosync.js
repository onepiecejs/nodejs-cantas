...
Backbone.sync = function (method, model, options) {
  ...
  var defer = $.Deferred();
  var changes = params.data;
  if (params.patch !== undefined && params.patch) {
    changes = params.attrs;
    changes['_id'] = params.data['_id'];
  }
  io.emit(namespace + ':' + method, changes, function (err, data) {
    if (err) {
      options.error(err);
      defer.reject();
    } else {
